...
//while (CONTROL_PIN == LOW); 
//runREAD();
    
  
  
  //TWBR = ((F_CPU / 100000L) - 16) / 2; // Set I2C frequency to 400kHz
  ...
  if(client) {
    ...
    if (command == "msg") {      
       msg = client.readStringUntil('/');             // read the incoming data
        File dataFile = FileSystem.open("/www/param4.txt",FILE_WRITE);
       dataFile.print(msg);
       dataFile.close();
